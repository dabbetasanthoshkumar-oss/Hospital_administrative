...
async function diagnostic() {
    ...
    console.log('\n--- Enum Check (Testing Patient Role) ---')
    const { error: testError } = await supabase.from('profiles').select('role').limit(1)
    if (testError) {
        console.error('Enum Check Error:', testError.message)
    } else {
        console.log('Enum check via profiles table works.')
    }
}

diagnostic().catch(err => console.error('Script Error:', err))
